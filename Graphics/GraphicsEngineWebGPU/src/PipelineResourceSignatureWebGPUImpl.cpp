...
void PipelineResourceSignatureWebGPUImpl::CreateBindGroupLayouts(const bool IsSerialized)
{
    ...
    // Total number of static inline constants (for static resource cache sizing)
    Uint32 TotalStaticInlineConstants = 0;
    ...
    if (StaticResourceCount != 0)
    {
        ...
        m_pStaticResCache->InitializeGroups(GetRawAllocator(), 1, &StaticResourceCount, TotalStaticInlineConstants);
    }
...
void PipelineResourceSignatureWebGPUImpl::InitSRBResourceCache(ShaderResourceCacheWebGPU& ResourceCache)
{
    ...
    if (m_NumInlineConstantBuffers > 0)
    {
        ...
        {
            const InlineConstantBufferAttribsWebGPU& InlineCBAttr = m_InlineConstantBuffers[i];
