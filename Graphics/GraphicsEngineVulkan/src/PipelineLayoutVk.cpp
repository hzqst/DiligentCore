                              ...
                              Uint32                                         SignatureCount) noexcept(false)
{
    ...
    for (Uint32 BindInd = 0; BindInd < SignatureCount; ++BindInd)
    {
        ...
        if (!m_PushConstantInfo)// && pSignature->HasInlineConstants()) //This comment need to be restored once we finished PipelineResourceSignatureVkImpl::HasInlineConstants
        {
